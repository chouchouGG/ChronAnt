...
import cn.uhoc.config.ThreadPoolConfigProperties;
...
@Slf4j
@EnableConfigurationProperties({AsyncTaskWithMultiStageConfig.class, ThreadPoolConfigProperties.class})  // 显式注册配置类
public abstract class AbstractLauncher implements Launch {

    @Resource
    protected AsyncTaskWithMultiStageConfig asyncTaskWithMultiStageConfig; // 暂时没有用到

    @Resource
    protected ThreadPoolConfigProperties threadPoolConfig;
     ...
     * 存储任务配置信息
     */
    protected Map<String, TaskCfgEntity> taskTypeConfigMap;
    ...
    public AbstractLauncher() {
        ...
        // 初始化拉取任务的偏移
        initOffset = new AtomicInteger(asyncTaskWithMultiStageConfig.getInitOffsetForScheduleTask()); // TODO 思考这里使用原子整型的原因？
        // 初始化拉取任务配置的时间周期
        updateTaskConfigPeriod = asyncTaskWithMultiStageConfig.getCycleUpdateScheduleTaskTypeConfigTime();
        // 存储任务配置信息
